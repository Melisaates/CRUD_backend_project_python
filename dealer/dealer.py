...
class Dealer:
    @router.get("/dealer1/{dealer_id}", status_code=status.HTTP_200_OK, tags=["dealer"])
    def get_dealer(self, dealer_id: int):
        sql = SqlRaw.paths(["api/routes/dealer/model"])
        sql.load("dealer.sql").connect(settings.conn)
    ...
    @router.get("/dealer2/{city_id}", status_code=status.HTTP_200_OK, tags=["dealer info"])
    def get_new_dealer(self, city_id: int):
        sql = SqlRaw.paths(["api/routes/dealer/model"])
        sql.load("new_dealer.sql").connect(settings.conn)
